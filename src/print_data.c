...
/*   Updated: 2019/02/12 21:08:28 by dzaporoz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	put_route(t_data *data, t_list *route, int route_index)
{
	...
	while (room)
	{
		if (room->content != START && room->content != END &&
			((t_room*)room->content)->back_link &&
			((t_room*)room->content)->back_link->next)
...
void		put_stat_data(t_data *data)
{
	...
	ft_printf("%-39s %d\n", "5. Number of used paths:",
			ft_lstcount(data->unique_paths));
